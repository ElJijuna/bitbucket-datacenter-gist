class GistManager {
  ...
  async createFile(filePath, content, branch) {
    ...
    if (existing !== null) throw Object.assign(new Error(`File already exists: ${filePath}`), { status: 409 });
  ...
  async updateFile(filePath, content, branch) {
    const existing = await this.gitManager.readFile(filePath, branch);
    if (existing === null) throw Object.assign(new Error(`File not found: ${filePath}`), { status: 404 });
    const contentStr = typeof content === 'string' ? content : JSON.stringify(content, null, 2);
